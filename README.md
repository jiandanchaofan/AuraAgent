...
| A new Skill | Just add `skills_store/<name>/` (`SKILL.md` + `run.py`) — auto-discovered on the next scan; or have the LLM propose it itself mid-conversation via `propose_new_skill`; or a human installs an external Skill package directly with `/skills load|install` |
