"""market_company_moves skill — searches Google News' public RSS feed for
recent major-company-activity headlines within a given market segment.
...
QUERY_SUFFIX = "(acquisition OR partnership OR earnings OR CEO OR strategy OR expansion)"
TIMEOUT_SECONDS = 10
...
def main() -> None:
    ...
    try:
        ...
        print(f"Error fetching company-moves news for '{segment}': {exc}")
        return

    if not headlines:
        print(f"No company-activity news found for market segment '{segment}'.")
        return

    print(f"Recent company-activity news for the '{segment}' market ({len(headlines)} result(s)):")
    for h in headlines:
        line = f"- {h['title']}"
        if h["source"]:
            line += f" ({h['source']})"
