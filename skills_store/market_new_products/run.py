"""market_new_products skill — searches Google News' public RSS feed for
recent new-product-launch headlines within a given market segment.
...
QUERY_SUFFIX = "(new product OR launch OR unveils OR announces)"
TIMEOUT_SECONDS = 10
...
def main() -> None:
    ...
    try:
        ...
        print(f"Error fetching new-product news for '{segment}': {exc}")
        return

    if not headlines:
        print(f"No new-product news found for market segment '{segment}'.")
        return

    print(f"Recent new-product news for the '{segment}' market ({len(headlines)} result(s)):")
    for h in headlines:
        line = f"- {h['title']}"
        if h["source"]:
            line += f" ({h['source']})"
